export const buildCommentNotificationEmail = (
  authorName: string,
  lead: { person_info: string; address: string; id: string },
  commentContent: string,
  origin: string,
) => ({
  subject: `Ny kommentar på en merknad du følger: ${lead.person_info}`,
  html: `
    <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 5px; overflow: hidden;">
      ...
      <div style="padding: 20px;">
        <h2 style="color: #4f46e5;">Ny kommentar</h2>
        <p><strong>${authorName}</strong> kommenterte på en merknad du er nevnt i for leadet <strong>${lead.person_info}</strong>.</p>
        <div style="background-color: #f9f9f9; padding: 15px; border-radius: 5px; margin: 20px 0; border-left: 3px solid #10b981;">
          <p style="margin: 0;">${commentContent.replace(/@\[([^\]]+)\]/g, "<strong>@$1</strong>")}</p>
        </div>
